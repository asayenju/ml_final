...
def main():
    ...
    print(
        f"Starting Random Forest 10-fold run | samples={len(y)} features={X.shape[1]} "
    ...
    for hp_idx, trees in enumerate(TREE_VALUES, start=1):
        hp_start = time.time()
        print(f"\n[{hp_idx}/{len(TREE_VALUES)}] trees={trees} | starting", flush=True)
        accs, precs, recs, f1s = [], [], [], []
        for fold_idx, (tr, te) in enumerate(skf.split(X, y), start=1):
            fold_start = time.time()
            print(f"  fold {fold_idx}/{K_FOLDS} | prep...", flush=True)
            X_train, X_test = X[tr], X[te]
            y_train, y_test = y[tr], y[te]
            X_train_p, X_test_p = preprocess_for_random_forest(X_train, X_test, numeric_cols)
            print(f"  fold {fold_idx}/{K_FOLDS} | fit model...", flush=True)
            ...
            print(f"  fold {fold_idx}/{K_FOLDS} | predict + metrics...", flush=True)
            ...
            f1s.append(f1)
            print(
                f"  fold {fold_idx}/{K_FOLDS} done | "
                f"acc={acc:.4f} prec={p:.4f} rec={r:.4f} f1={f1:.4f} "
        ...
        print(
            f"[{hp_idx}/{len(TREE_VALUES)}] trees={trees} done | "
            f"acc={np.mean(accs):.4f}, prec={np.mean(precs):.4f}, rec={np.mean(recs):.4f}, f1={np.mean(f1s):.4f} "
            f"| hp_elapsed={time.time()-hp_start:.1f}s",
            flush=True,
        )

    out_dir = ROOT / "results" / "rice"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_csv = out_dir / "random_forest_hyperparams_10fold.csv"
    ...
    for metric, color in [("accuracy", "#1f77b4"), ("precision", "#ff7f0e"), ("recall", "#2ca02c"), ("f1", "#d62728")]:
        ...
        ax.plot(x, yv, marker="o", color=color, linewidth=2)
        ax.fill_between(x, np.array(yv)-np.array(ys), np.array(yv)+np.array(ys), color=color, alpha=0.15)
        ax.set_xlabel("Number of trees")
        ax.set_ylabel(metric.upper())
        ax.set_ylim(0, 1.05)
        ax.set_title(f"Random Forest ({K_FOLDS}-fold) on Rice: {metric.upper()} vs trees")
        ax.grid(True, linestyle="--", alpha=0.5)
        fig.tight_layout()
        fig.savefig(out_dir / f"random_forest_{metric}_vs_trees_10fold.png", dpi=150)
        plt.close(fig)

    print(f"Saved: {out_csv}")
    print(f"Saved plots in: {out_dir}")
    print(f"Total elapsed: {time.time()-run_start:.1f}s", flush=True)
