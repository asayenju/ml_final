...
def prune_tree_to_max_depth(node, max_depth, current_depth=0):
    ...
    for val, child in node["children"].items():
        pruned["children"][val] = prune_tree_to_max_depth(child, max_depth, current_depth + 1)
    return pruned
...
def main():
    ...
    for depth in DEPTH_VALUES:
        ...
        for tr, te in skf.split(X, y):
            ...
            X_train_p, X_test_p = preprocess_for_decision_tree(X_train, X_test, numeric_cols, DISCRETIZATION_BINS)
            model = DecisionTreeClassifierScratch().fit(X_train_p, y_train)
            model.tree_ = prune_tree_to_max_depth(copy.deepcopy(model.tree_), max_depth=depth, current_depth=0)
    ...
    for metric, color in [("accuracy", "#1f77b4"), ("precision", "#ff7f0e"), ("recall", "#2ca02c"), ("f1", "#d62728")]:
        ...
        ax.set_ylim(0, 1.05)
        ax.set_title(f"Decision Tree ({K_FOLDS}-fold) on Parkinsons: {metric.upper()} vs depth")
